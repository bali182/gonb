...
import { enharmonic, get, pitchClass, transpose } from '@tonaljs/note'
...
export function getUnfrettedPresetOption(
  ...
  const notes = tuning
    .map((note) => pitchClass(note))
    .map((note) => beautifyNote(note, language))
  return {
    label: t('PresetNames.Unfretted', { notes: notes.join(', ') }),
